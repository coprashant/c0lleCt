...
Game::Game() : window(sf::VideoMode(1000, 800), "c0lleCt"), score(0), eggSpeed(0.8f), gameOver(false), isPaused(false) {
    ...
    backgroundWidth = window.getSize().x; // Full window width
    backgroundHeight = backgroundTexture.getSize().y * backgroundSprite.getScale().y;
    ...
    // Load and setup the egg texture
    if (!eggTexture.loadFromFile("resources/images/egg.png")) {
    std::cerr << "Failed to load egg texture" << std::endl;
    }
    ...
    continueButtonText.setPosition(window.getSize().x / 2.0f - 100, window.getSize().y / 2.0f - 70);
    ...
    endGameButtonText.setPosition(window.getSize().x / 2.0f - 100, window.getSize().y / 2.1f + 20);

    // Setup Play Button Shadow
    playButtonShadow.setSize(playButton.getSize());
    playButtonShadow.setPosition(playButtonPosition + sf::Vector2f(250, 80));  // Shadow offset
    playButtonShadow.setFillColor(sf::Color(0, 0, 0, 150));  // Semi-transparent black

    // Setup Exit Button Shadow
    exitButtonShadow.setSize(exitButton.getSize());
    exitButtonShadow.setPosition(exitButtonPosition + sf::Vector2f(250, 80));  // Shadow offset
    exitButtonShadow.setFillColor(sf::Color(0, 0, 0, 150));  // Semi-transparent black


    playButtonPosition = sf::Vector2f((window.getSize().x - playButton.getSize().x) / 2.0f, window.getSize().y / 2.0f - 100);
    exitButtonPosition = sf::Vector2f((window.getSize().x - exitButton.getSize().x) / 2.0f, playButtonPosition.y + playButton.getSize().y + 20);


    playButton.setSize(sf::Vector2f(200, 60));
    playButton.setPosition(
    (window.getSize().x - playButton.getSize().x) / 2.0f,
    ...
    (window.getSize().x - exitButton.getSize().x) / 2.0f,  // Center horizontally
    playButton.getPosition().y + playButton.getSize().y + 20 // Below Play button
);
...
void Game::renderMenu() {
    ...
    // Center the text on buttons dynamically
    sf::FloatRect playTextBounds = playButtonText.getLocalBounds();
    playButtonText.setOrigin(playTextBounds.left + playTextBounds.width / 2.0f,
                             playTextBounds.top + playTextBounds.height / 2.0f);
...
void Game::run() {
    ...
    while (window.isOpen()) {
        ...
        if (isMenuActive) {
            ...
            float deltaTime = clock.restart().asSeconds();  // Get elapsed time since the last frame
    ...
    while (window.pollEvent(event)) {
        ...
        if (isMenuActive) {
            ...
            if (event.type == sf::Event::Resized) {
                resizeView();
                backgroundWidth = window.getSize().x;
                backgroundHeight = backgroundTexture.getSize().y * backgroundSprite.getScale().y;
            }
            ...
            if (event.type == sf::Event::MouseButtonPressed && !gameOver) {
                if (pauseButton.getGlobalBounds().contains(window.mapPixelToCoords(sf::Mouse::getPosition(window)))) {
                    isPaused = !isPaused;
                    pauseButtonText.setString(isPaused ? ">" : "| |");
                }
            }
...
void Game::update(float deltaTime) {
    ...
    float basketSpeed = 700.0f; // Pixels per second
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left) || sf::Keyboard::isKeyPressed(sf::Keyboard::A)) {
        basket.move(-basketSpeed * deltaTime, window.getSize().x);
    }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Right) || sf::Keyboard::isKeyPressed(sf::Keyboard::D)) {
        basket.move(basketSpeed * deltaTime, window.getSize().x);
    }

    if (eggs.empty() || eggs.back().getBounds().top > 300) {
    ...
    static int previousLevel = level; // Track the previous level to adjust speed only when the level increases
...
void Game::spawnEgg(float xPos, float yPos) {
    int level = score / 10 + 1;
    int eggCount = std::ceil(level / 3.0f);

    if (eggs.size() < eggCount) {
        float eggWidth = eggTexture.getSize().x * 0.4f; 
        xPos = std::max(0.0f, std::min(xPos, (float)window.getSize().x - eggWidth));

        // Pass the eggTexture we loaded earlier!
        eggs.emplace_back(xPos, yPos, window.getSize(), eggSpeed, eggTexture);
    }
}
